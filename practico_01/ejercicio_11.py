...
def suma_cubo_pares_sum_list(numeros: Iterable[int]) -> int:
    ...
    return sum(list(n for n in map(lambda x: x**3, numeros) if n % 2 == 0))
...
numeros_ordenada = sorted(list (n for n in numeros if n % 2 != 0)) + sorted(list (n for n in numeros if n % 2 == 0))  
